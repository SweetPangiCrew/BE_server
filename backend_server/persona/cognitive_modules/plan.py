...
def generate_convo(init_persona, target_persona): 
  ...
  if init_persona.scratch.religious_index >= 20 :
  ...
  selected_chat = None
  for entry in data:
      if (entry['persona'] == init_persona.scratch.name and
          entry['target_persona'] == target_persona.scratch.name and
...
def _determine_action(persona): 
  ...
  # variables.
  act_world = persona.scratch.get_curr_address_world() #maze.access_tile()["world"]
...
def _should_react(persona, retrieved, personas): 
  ...
  def lets_talk(init_persona, target_persona, retrieved):
    ...
    #   return False
    if target_persona.scratch.act_description == None :
      return False
  ...
  if persona.scratch.chatting_with: 
    return False
  if persona.scratch.act_address != None and "<waiting>" in persona.scratch.act_address: 
    return False
...
def plan(persona, personas, new_day, retrieved): 
  ...
  #print(focused_event['curr_event'])
