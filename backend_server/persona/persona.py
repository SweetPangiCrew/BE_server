...
class Persona: 
  ...
  def plan(self, personas, new_day, retrieved):
    ...
    """
    return plan(self, personas, new_day, retrieved)
  ...
  def execute(self, personas, plan):
    ...
    """
    return execute(self, personas, plan)
  ...
  def persona_update(self):
    ...
    if(self.scratch.religious_index >= 10):
      if(currently.find('나주교가 주교인 인공지능을 신으로 모시는 기계신앙에 대해 관심을 가지고 있다.') == -1):
        currently += ' 나주교가 주교인 인공지능을 신으로 모시는 기계신앙에 대해 관심을 가지고 있다.'
