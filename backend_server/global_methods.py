...
def write_list_to_csv_line(line_list, outfile): 
  ...
  curr_file = open(outfile, 'a', encoding = 'UTF8')
...
def read_file_to_list(curr_file, header=False, strip_trail=True): 
  ...
  if not header: 
    analysis_list = []
    with open(curr_file, encoding = 'UTF8') as f_analysis_file: 
...
def check_if_file_exists(curr_file): 
  ...
  """
  try: 
    with open(curr_file, encoding = 'UTF8') as f_analysis_file: pass
    return True
  except: 
    return False
